...
const Player = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((PlayerRes) => {
        ...
        console.log("HEEEERE", PlayerRes.data.progress_ms);
  ...
  }, [track, prog_ms, dur_ms, trackChange]);
    ...
    <div className='progress'>
      {playing.is_playing ? (
        <p>
          ...
          <br /> {track} - {playing.item.artists[0].name}{" "}
        </p>
      ) : (
        <p>Currently Playing: Nothing</p>
      )}
      {playing.is_playing ? (
        <p>
          From: <br /> {playing.item.album.name}{" "}
        </p>
      ) : (
        <p></p>
      )}
